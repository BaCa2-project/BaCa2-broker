...
class DetailedTests(ut.TestCase):
    ...
    def setUp(self) -> None:
        self.host = '127.0.0.1'
        self.port = 8080
        self.server = KolejkaCommunicationServer(self.host, self.port)
    ...
    def test_add_submit(self) -> None:
        ...
        self.assertEqual(len(self.server), 1000)

    def test_release_submit(self) -> None:
        for i in range(1000):
            self.server.add_submit("%05d" % i)

        def release_after_time(time: float, sub_id: str):
            sleep(time)
            self.server.release_submit(sub_id)
        ...
        for i in self.server.submit_dict.values():
    ...
    def test_await_submit(self) -> None:
        ...
        def await_after_time(time: float, sub_id: str):
            sleep(time)
            self.server.await_submit(sub_id)
        ...
        self.assertEqual(len(self.server), 0)
    ...
    def test_add_await_release_errors(self) -> None:
        ...
        self.assertTrue(len(self.server.submit_dict) == 1000)
        # check if an exception is raised while trying to add duplicate submits
        for i in range(1000):
            self.assertRaises(ValueError, self.server.add_submit, "%05d" % i)

        def impatient_student():
            # waits for half of the submits
            for j in reversed(submits[len(submits)//2:]):
                out = self.server.await_submit(j, 5.5)
                self.assertTrue(out)
            for j in submits[:len(submits)//2]:
                self.server.await_submit(j, 0.01)

        def kolejka():
            # release all submits
            for j in reversed(submits):
                self.server.release_submit(j)
                sleep(0.01)
            # check if an exception is raised
            for j in submits:
                self.assertRaises((ValueError, KeyError), self.server.release_submit, j)
        ...
        print("test_add_await_release_errors: %d awaited for" % (1000 - len(self.server)))
        self.assertTrue(len(self.server) <= 1000//2)
    ...
    def general(self, submit_number: int, wait_interval: float, log: bool = False) -> None:
        ...
        def send_submits():
            ...
            while True:
                ...
                self.server.add_submit(current)
                if log:
        ...
        self.assertFalse(self.server.submit_dict)
